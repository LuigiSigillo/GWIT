...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def preprocess_train(examples):
        ...
        #EEG
        conditioning_images = [torch.tensor(image) for image in examples[conditioning_image_column]] #EEG
...
def collate_fn(examples):
    ...
    subjects = torch.stack([example["subject"] for example in examples]) if "ALL" in args.dataset_name else torch.tensor([4])
    return {
