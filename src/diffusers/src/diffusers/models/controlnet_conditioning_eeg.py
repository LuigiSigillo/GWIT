...
class ControlNetEEGConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in = nn.Conv1d(conditioning_channels, block_out_channels[0], kernel_size=1, stride=1)
        self.blocks = nn.ModuleList([])

        for i in range(len(block_out_channels) - 1):
            channel_in = block_out_channels[i]
            channel_out = block_out_channels[i + 1]
            self.blocks.append(nn.Conv1d(channel_in, channel_in, kernel_size=1, ))
            self.blocks.append(nn.Conv1d(channel_in, channel_out, kernel_size=block_strides[i + 1], stride=block_strides[i + 1]))
